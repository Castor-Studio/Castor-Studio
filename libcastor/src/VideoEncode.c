...
CASTOR_CORE_API int video_encoder_encode_frame(VideoEncoder* enc, AVFrame* src, FILE* out) {
    ...
    /* Première frame — memoriser l'origine */
    if (!enc->first_pts_set) {
        enc->first_pts     = src->pts;
        enc->first_pts_set = 1;
    }

    /* PTS relatif au premier frame, converti en time_base encodeur */
    enc->frame->pts = enc->frame_index++;

    // fprintf(stderr, "[PTS] time_base=%d/%d pts=%lld delta_us=%lld\n",
    //     enc->ctx->time_base.num, enc->ctx->time_base.den,
    //     (long long)enc->frame->pts,
    //     (long long)(src->pts - enc->first_pts));
